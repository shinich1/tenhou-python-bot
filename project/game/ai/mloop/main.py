...
from game.ai.mloop.defence.main import DefenceHandler
from game.ai.mloop.hand_builder import HandBuilder
from game.ai.mloop.riichi import Riichi
from game.ai.mloop.strategies.chiitoitsu import ChiitoitsuStrategy
from game.ai.mloop.strategies.chinitsu import ChinitsuStrategy
from game.ai.mloop.strategies.formal_tempai import FormalTempaiStrategy
from game.ai.mloop.strategies.honitsu import HonitsuStrategy
from game.ai.mloop.strategies.main import BaseStrategy
from game.ai.mloop.strategies.tanyao import TanyaoStrategy
from game.ai.mloop.strategies.yakuhai import YakuhaiStrategy
...
class ImplementationAI(InterfaceAI):
    ...
    def try_to_call_meld(self, tile_136, is_kamicha_discard, remaining_tiles):
        ...
        meld, discard_option = self.current_strategy.try_to_call_meld(tile_136, is_kamicha_discard, tiles_136, remaining_tiles)
        if discard_option:
    ...
    def determine_strategy(self, tiles_136):
        ...
        if self.player.table.has_open_tanyao:
            strategies.append(TanyaoStrategy(BaseStrategy.TANYAO, self.player,self.gpparams))

        strategies.append(YakuhaiStrategy(BaseStrategy.YAKUHAI, self.player,self.gpparams))
        strategies.append(HonitsuStrategy(BaseStrategy.HONITSU, self.player,self.gpparams))
        strategies.append(ChinitsuStrategy(BaseStrategy.CHINITSU, self.player,self.gpparams))

        strategies.append(ChiitoitsuStrategy(BaseStrategy.CHIITOITSU, self.player,self.gpparams))
        strategies.append(FormalTempaiStrategy(BaseStrategy.FORMAL_TEMPAI, self.player,self.gpparams))
